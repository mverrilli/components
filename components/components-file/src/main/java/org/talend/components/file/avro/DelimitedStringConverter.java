...
public class DelimitedStringConverter implements AvroConverter<String, IndexedRecord> {
	...
	/**
	 * Design schema. It is specified by user
	 */
	private final Schema designSchema;
	
	/**
	 * Runtime schema. Output IndexedRecords contain this runtime schema
	 */
	...
	public IndexedRecord convertToAvro(String delimitedString) {
		...
		IndexedRecord record = new GenericData.Record(designSchema);
		for (int i = 0; i < size; i++) {
	...
	public String convertToDatum(IndexedRecord record) {
		if (!designSchema.equals(record.getSchema())) {
